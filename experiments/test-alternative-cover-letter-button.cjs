// Test that both playwright and puppeteer scripts support the new alternative cover letter button markup
// with data-qa="add-cover-letter" attribute

const fs = require('fs');
const path = require('path');

console.log('🔍 Testing support for alternative cover letter button markup...\n');
...
if (playwrightCode.includes('data-qa="add-cover-letter"')) {
  console.log('✅ Playwright: Found support for data-qa="add-cover-letter" attribute');
} else {
  console.error('❌ Playwright: Missing support for data-qa="add-cover-letter" attribute');
  hasErrors = true;
}
...
if (playwrightCoverLetterLine) {
  ...
  if (playwrightCoverLetterLine.includes('add-cover-letter')) {
    console.log('✅ Playwright: Selector includes new data-qa attribute');
  } else {
    console.error('❌ Playwright: Selector missing new data-qa attribute');
    hasErrors = true;
  }
} else {
...
if (puppeteerCode.includes('add-cover-letter')) {
  console.log('✅ Puppeteer: Found support for add-cover-letter value');
} else {
  console.error('❌ Puppeteer: Missing support for add-cover-letter value');
  hasErrors = true;
}
...
if (puppeteerCoverLetterSection.includes('Добавить сопроводительное') &&
    puppeteerCoverLetterSection.includes('add-cover-letter')) {
  console.log('✅ Puppeteer: Cover letter logic checks both text and data-qa attribute');
} else {
...
if (hasErrors) {
  ...
  console.log('✅ All checks passed! Both scripts support the new alternative cover letter button markup.');
}
